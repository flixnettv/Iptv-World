...
async function loadApps() {
    ...
    try {
        ...
        if (apps.length === 0) {
            container.innerHTML = '<p class="no-apps">لا توجد تطبيقات متاحة حالياً</p>';
            return;
        }
        ...
        container.innerHTML = apps.map((app, index) => `
            <article class="app-card" style="animation: fadeInUp 0.5s ease ${index * 0.1}s both">
                <div class="app-image">
                    <img src="${app.image_url || 'assets/img/placeholder.png'}" 
                ...
                <div class="app-info">
                    ...
                    <div class="app-price">${app.price > 0 ? `${app.price} ${app.currency || 'USD'}/year` : 'مجاني'}</div>
